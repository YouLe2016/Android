...
class EasyDialog : DialogFragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setStyle(STYLE_NO_TITLE, R.style.EasyDialog)
    ...
    private fun initWindowParams() {
        ...
        if (gravity != Gravity.CENTER) {
            ...
            if (gravity == Gravity.BOTTOM) {
                lp.windowAnimations = R.style.EasyDialog_Bottom_Anim
            } else if (gravity == Gravity.TOP) {
                lp.windowAnimations = R.style.EasyDialog_top_Anim
            }
        }
        if (margin != 0) {
            val marginTotal = 2 * TypedValue.applyDimension(
